<template>
    <div class="page" id="billPage"> 
        ...
        <div id="billDetailContainer" class="container-top" v-infinite-scroll="driverloadMore()" infinite-scroll-immediate-check="false" infinite-scroll-disabled="loading" infinite-scroll-distance="50">
...
export default {
    ...
    ready (){
        ...
        //列表框的最小高度
        self.$nextTick(()=>{
            self.listViewH = $("#billDetailContainer").height() - 10;
        })
    ...
    events:{
        ...
        taskDetailShow(index,value,ttId){//显示任务详情页面
            ...
            self.go('/billOrderList', ttId); 
        },
    },
    methods: {
        backPage(){
            let self = this;
            let backPath = Tokens.fetch("billDetailBackPath");
            self.back(backPath);
        },
        go (link, param) {
            let self = this
    ...
    route: {
        data ({ from, to, next }) {
            ...
            self.isLoadId = null;
            if(from.path && from.path.indexOf('billOrderList')!= -1 && self.driverList.length != 0){     
                let preSrollTop = Tokens.fetch("billDetailContainer");
                self.$nextTick(()=>{
                    $('#billDetailContainer').animate({scrollTop:preSrollTop},0); 
                })
            }else{
                ...
                self.getDriverData();
                self.$nextTick(()=>{
                    $('#billDetailContainer').animate({scrollTop:0},0); 
                })
            }

            //需要返回的路径
            if(from.path && ((from.path.indexOf('accountMsgList')!= -1) || (from.path.indexOf('accountIndex')!= -1))){
                Tokens.save("billDetailBackPath",'');
                Tokens.save("billDetailBackPath",from.path);
            }
            next();
        },
        deactivate ({ next }) {
        ...
        canDeactivate({to, from, next}) {
            let self = this;
            let scrollTop = $("#billDetailContainer").scrollTop();
            Tokens.save("billDetailContainer",scrollTop);
            next()
        },
